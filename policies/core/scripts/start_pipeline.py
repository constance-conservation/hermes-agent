...
def _materialize_workspace_assets(
    ...
    """Merge generated + runtime-agent policy subtrees into the canonical policy root.

    When ``policy_root`` is set (typical: ``AGENT_HOME/policies``), writes there — the same
    tree produced by ``_materialize_canonical_policy_root``. Legacy fallback: only
    ``workspace_root`` and no ``policy_root`` uses ``workspace_root/policies``.
    """
    ...
    policies_dst = policy_root if policy_root is not None else workspace_root / "policies"
    ...
    print(f"start_pipeline: materialized runtime-editable policy subtrees under {policies_dst}")
    return 0
...
def _write_workspace_layout_md(workspace_root: Path, policy_root: Path | None) -> None:
    ...
    if policy_root is not None:
        pr_res = policy_root.resolve()
        policy_tree_line = (
            f"- **Canonical policy tree (full bundle + generated + runtime/agent copies):** `{pr_res}`\n"
        )
    else:
        policy_tree_line = (
            f"- **Nested policies tree (legacy fallback when `--policy-root` omitted):** `{wr}/policies/`\n"
        )
...
- **Workspace root:** `{wr}`
{pr_line}{policy_tree_line}- **Operations:** `{wr}/operations/` (org registers; `projects/` for per-project trees)

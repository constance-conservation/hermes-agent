...
> **Governance read order** — step 3 of 60 in the canonical `policies/` sequence (layer map & tables: [`README.md`](../README.md)).
...
- **`scripts/droplet_remote_venv.sh`** is sourced by **`ssh_droplet.sh`**, **`ssh_droplet_user.sh`**, and **`ssh_droplet_hermesuser_direct.sh`**. It prepends a remote shell snippet: **`cd`** to **`HERMES_DROPLET_REPO`** (default **`/home/hermesuser/hermes-agent`**), then **`source venv/bin/activate`** when **`venv/bin/activate`** exists.  

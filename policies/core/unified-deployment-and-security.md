...
**Workstation prerequisites:** Encrypted private key, port, host, and **`SSH_USER`** in the same env file **`ssh_droplet.sh`** reads. **`hermes … droplet`** (**`scripts/agent-droplet`**) sets **`HERMES_DROPLET_WORKSTATION_CLI=1`**, which makes **`ssh_droplet.sh`** ignore **`HERMES_DROPLET_ALLOW_ENV_PASSPHRASE`** / **`SSH_PASSPHRASE`** from that file so the **SSH key passphrase is typed interactively** (no env-file unlock). Remote Hermes runs with **`HERMES_HOME`** defaulting to **`…/profiles/chief-orchestrator`** and **`./venv/bin/python`** on the server (create the venv on the VPS if missing). If **`SSH_USER`** equals the runtime user (**`hermesadmin`** by default), no **`sudo`** step runs; otherwise **`sudo -u`** prompts on the remote TTY (**no** **`SSH_SUDO_PASSWORD`** / **`sudo -S`** for this path). For automation-only SSH (no interactive key/sudo), call **`scripts/ssh_droplet.sh`** directly without **`HERMES_DROPLET_WORKSTATION_CLI`**. **`scripts/ssh_droplet_user.sh`** also uses interactive **`sudo`**. **`agent-droplet`** sets **`HERMES_DROPLET_INTERACTIVE=1`** so the TTY gate passes under IDEs.

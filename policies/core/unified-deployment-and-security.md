...
**Short PATH commands** (this repo’s **`scripts/`** on `PATH`, e.g. direnv): **`droplet`** and **`droplet-run`** wrap **`agent-droplet`** and **`droplet_run.sh`** so you do not type full paths.

...
from utils import atomic_yaml_write, normalize_line_endings
...
class GatewayRunner:
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        try:
            ...
            if _pf_slug:
                if not resolve_profile_home_for_gateway(_pf_slug):
                    ...
                    if _bad_ad:
                        _bm = {"thread_id": source.thread_id} if source.thread_id else None
                        await _bad_ad.send(
                            source.chat_id,
                            f"⚠️ Unknown Hermes profile `{_pf_slug}`. "
                            f"Create it with `hermes profile create` or use `/profile list`.",
                            metadata=_bm,
                        )
                    return
                message_text = _mt2
                event.hermes_profile_slug = _pf_slug
        except Exception as _pf_exc:
            logger.debug("profile @mention parse skipped: %s", _pf_exc)
        ...
        try:
            ...
            agent_result = await self._run_agent(
                ...
                hermes_profile_slug=getattr(event, "hermes_profile_slug", None),
            )
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            _ex = ExitStack()
            _session_db_for_agent = self._session_db
            _cache_key_eff = session_key
            if hermes_profile_slug:
                from hermes_cli.profiles import resolve_profile_home_for_gateway
                from hermes_constants import get_hermes_home, set_hermes_home_override, reset_hermes_home_override
                from hermes_state import SessionDB

                _ph = resolve_profile_home_for_gateway(hermes_profile_slug)
                if not _ph:
                    return {
                        "final_response": (
                            f"⚠️ Unknown Hermes profile `{hermes_profile_slug}`. "
                            "Use `/profile list` where supported."
                        ),
                        "messages": [],
                        "api_calls": 0,
                        "tools": [],
                    }
                _tok = set_hermes_home_override(_ph)
                _session_db_for_agent = SessionDB(db_path=get_hermes_home() / "state.db")
                _cache_key_eff = f"{session_key}\x00profile\x00{hermes_profile_slug}"
                _ex.callback(lambda t=_tok: reset_hermes_home_override(t))
        ...
        try:
            ...
            if pending:
                ...
                return await self._run_agent(
                    ...
                    hermes_profile_slug=hermes_profile_slug,
                )
        finally:

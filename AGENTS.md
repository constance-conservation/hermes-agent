...
**direnv (`.envrc`):** Prefer **`./venv/bin/python`** for `HERMES_REAL_BIN` (or `./venv/bin/hermes`). Do **not** run `command -v hermes` *before* `PATH_add scripts/core` — that captures a **global** install. The repo’s **`scripts/core/hermes`** shim runs **`venv/bin/python -m hermes_cli.main`** first (avoids a stale **`venv/bin/hermes`** shebang if the checkout moved). **Trailing `droplet` (VPS hop)** is handled inside **`hermes_cli.main`**: **`hermes droplet`**, **`hermes tui droplet`**, **`hermes doctor droplet`**, etc. exec the bundled **`hermes_cli/scripts/core/agent-droplet`** (also symlinked as **`scripts/core/agent-droplet`** in the git tree). Pip/wheel installs include those files next to the package — you do **not** need **`scripts/core`** on `PATH` ahead of **`venv/bin`** for the hop to work. **`hermes tui`** is an alias for **`hermes chat`** (interactive CLI). **`hermes droplet --help`** documents the subcommand; **`droplet`** is not a valid profile name (`hermes -p droplet` is wrong).

**Shell outside the repo (Mac/Linux):** To make **`hermes`** always use this checkout’s venv and shim (including **`hermes … droplet`**), source **`scripts/shell/hermes-env.sh`** from **`~/.zshrc`** / **`~/.bashrc`** after setting **`HERMES_AGENT_REPO`** if your clone is not **`$HOME/hermes-agent`**. That defines **`hermes()`** (delegates to **`scripts/core/hermes`**), **`droplet()`** (admin **`SSH_USER`** then **`sudo`** to **`hermesuser`** — **`scripts/core/ssh_droplet_user.sh`**), and **`droplet_direct()`** (one-hop **`ssh hermesuser@…`** when your pubkey is in **`hermesuser`**’s **`authorized_keys`** — **`scripts/core/ssh_droplet_hermesuser_direct.sh`**). **`~/.env/.env`**: **`SSH_PORT`**, **`SSH_TAILSCALE_IP`** or **`SSH_IP`**, **`SSH_USER`** (admin); **`~/.env/.ssh_key`**. Remote **`hermesuser`** shells and one-shots activate the checkout **`venv`** via **`scripts/core/droplet_remote_venv.sh`** (optional **`HERMES_DROPLET_REPO`**, **`HERMES_DROPLET_VENV_USER`**); see **`policies/core/unified-deployment-and-security.md`** Step 15 — **Remote venv**. On the VPS, **`agent-droplet`** runs **`./venv/bin/python -m hermes_cli.main`** and exports **`VIRTUAL_ENV`** + **`PATH`** so child processes keep using that venv.

### Droplet SSH (automation vs interactive Hermes)

- **VPS hop (suffix only):** put **`droplet` as the last argument** — e.g. **`hermes droplet`**, **`hermes doctor droplet`**, **`hermes tui droplet`**. There is **no** `droplet` profile; do not set **`~/.hermes/active_profile`** to **`droplet`**. From the repo, **`PATH_add scripts/core`** makes **`scripts/core/hermes`** the preferred `hermes` binary (it delegates when the last token is **`droplet`**).

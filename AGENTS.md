...
**direnv (`.envrc`):** Prefer **`./venv/bin/python`** for `HERMES_REAL_BIN` (or `./venv/bin/hermes`). Do **not** run `command -v hermes` *before* `PATH_add scripts/core` — that captures a **global** install. The repo’s **`scripts/core/hermes`** shim runs **`venv/bin/python -m hermes_cli.main`** first (avoids a stale **`venv/bin/hermes`** shebang if the checkout moved). **`PATH_add scripts/shell`** exposes **`droplet`** and **`droplet_direct`**: bare **`droplet`** opens an interactive SSH session as **`hermesuser`** (via **`scripts/core/ssh_droplet_user.sh`**); **`droplet git status`** runs one remote command as **`hermesuser`**. **Trailing `droplet` (Hermes VPS hop)** is different: handled inside **`hermes_cli.main`** when **`droplet` is the last argument** — **`hermes doctor droplet`**, **`hermes tui droplet`**, etc. exec **`agent-droplet`**. Pip/wheel installs include those files next to the package — you do **not** need **`scripts/core`** on `PATH` ahead of **`venv/bin`** for the hop to work. **`hermes tui`** is an alias for **`hermes chat`** (interactive CLI). **`hermes droplet --help`** documents the subcommand; **`droplet`** is not a valid profile name (`hermes -p droplet` is wrong).

**Shell outside the repo (Mac/Linux):** With direnv in the checkout, **`droplet`** / **`droplet_direct`** are already on **`PATH`**. Else set **`HERMES_AGENT_REPO`** and source **`scripts/shell/hermes-env.sh`** from **`~/.zshrc`** / **`~/.bashrc`** (default repo: **`$HOME/hermes-agent`**). That defines **`hermes()`** (delegates to **`scripts/core/hermes`**), **`droplet()`**, and **`droplet_direct()`** (same behavior as the **`scripts/shell/droplet`** helpers). **`~/.env/.env`**: **`SSH_PORT`**, **`SSH_TAILSCALE_IP`** or **`SSH_IP`**, **`SSH_USER`** (admin); **`~/.env/.ssh_key`**. Remote **`hermesuser`** shells and one-shots activate the checkout **`venv`** via **`scripts/core/droplet_remote_venv.sh`** (optional **`HERMES_DROPLET_REPO`**, **`HERMES_DROPLET_VENV_USER`**); see **`policies/core/unified-deployment-and-security.md`** Step 15 — **Remote venv**. On the VPS, **`agent-droplet`** runs **`./venv/bin/python -m hermes_cli.main`** and exports **`VIRTUAL_ENV`** + **`PATH`** so child processes keep using that venv.

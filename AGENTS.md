...
- **Tailscale SSH timeout, LAN works:** set **`MACMINI_SSH_LAN_IP`** (mini’s LAN IPv4) for fallback. When **`MACMINI_SSH_LAN_IP`** is set, **`ssh_operator.sh`** / **`ssh-operator-breakglass.sh`** default to **LAN first** (avoids waiting out **ConnectTimeout** on **100.x** when workstation Tailscale is down). Set **`MACMINI_SSH_TRY_LAN_FIRST=0`** in **`~/.env/.env`** if you want **Tailscale first** (e.g. always remote, no LAN path). To refresh **`MACMINI_SSH_HOST`**, on the mini run **`tailscale ip -4`** (or **`./scripts/core/operator_print_mini_tailscale_ip.sh`**) and update the env file.
- **LAN `Permission denied (publickey)` but Tailscale OK:** the **`operator`** account on the mini often has **`from="100.64.0.0/10,…"`** on the **`authorized_keys`** line — connections from **`192.168.*`** then fail **publickey** while **`100.*`** works. Fix on the mini: add your LAN CIDR to **`from=`**, or a **second** line with the same pubkey and a LAN **`from=`**, or use the breakglass key line **without** **`from=`** (weaker). Workaround from the laptop: **`MACMINI_SSH_TRY_LAN_FIRST=0`** so **`ssh_operator`** probes Tailscale first. **Encrypted `MACMINI_SSH_KEY` + two hosts:** probes default to **`BatchMode=yes`**; set **`HERMES_OPERATOR_ALLOW_ENV_PASSPHRASE=1`** (or **`HERMES_DROPLET_ALLOW_ENV_PASSPHRASE=1`**) and **`SSH_PASSPHRASE`** in **`~/.env/.env`**, or run **`operator`** from an **interactive** terminal ( **`HERMES_OPERATOR_SSH_NO_TTY` unset** ) so probes can prompt.

...
**direnv (`.envrc`):** Prefer **`./venv/bin/python`** for `HERMES_REAL_BIN` (or `./venv/bin/hermes`). Do **not** run `command -v hermes` *before* `PATH_add scripts` — that captures a **global** install. The repo’s **`scripts/hermes`** shim runs **`venv/bin/python -m hermes_cli.main`** first (avoids a stale **`venv/bin/hermes`** shebang if the checkout moved). **Trailing `droplet` (VPS hop)** is handled inside **`hermes_cli.main`** as well: **`venv/bin/hermes tui droplet`** and **`./venv/bin/python -m hermes_cli.main doctor droplet`** exec **`scripts/agent-droplet`** when that script exists in the same checkout — you do **not** need `scripts/` first on `PATH` for the hop to work. **`hermes droplet --help`** documents the subcommand; **`droplet`** is not a valid profile name (`hermes -p droplet` is wrong).

**Shell outside the repo (Mac/Linux):** To make **`hermes`** always use this checkout’s venv and shim (including **`hermes … droplet`**), source **`scripts/shell/hermes-env.sh`** from **`~/.zshrc`** / **`~/.bashrc`** after setting **`HERMES_AGENT_REPO`** if your clone is not **`$HOME/hermes-agent`**. That defines **`hermes()`** (delegates to **`scripts/hermes`**), **`droplet()`** (admin **`SSH_USER`** then **`sudo`** to **`hermesuser`** — **`scripts/ssh_droplet_user.sh`**), and **`droplet_direct()`** (one-hop **`ssh hermesuser@…`** when your pubkey is in **`hermesuser`**’s **`authorized_keys`** — **`scripts/ssh_droplet_hermesuser_direct.sh`**). **`~/.env/.env`**: **`SSH_PORT`**, **`SSH_TAILSCALE_IP`** or **`SSH_IP`**, **`SSH_USER`** (admin); **`~/.env/.ssh_key`**. Remote **`hermesuser`** shells and one-shots activate the checkout **`venv`** via **`scripts/droplet_remote_venv.sh`** (optional **`HERMES_DROPLET_REPO`**, **`HERMES_DROPLET_VENV_USER`**); see **`policies/core/unified-deployment-and-security.md`** Step 15 — **Remote venv**. On the VPS, **`agent-droplet`** runs **`./venv/bin/python -m hermes_cli.main`** and exports **`VIRTUAL_ENV`** + **`PATH`** so child processes keep using that venv.

### Droplet SSH (automation vs interactive Hermes)

- **VPS hop (suffix only):** put **`droplet` as the last argument** — e.g. **`hermes droplet`**, **`hermes doctor droplet`**, **`hermes tui droplet`**. There is **no** `droplet` profile; do not set **`~/.hermes/active_profile`** to **`droplet`**. From the repo, **`PATH_add scripts`** makes **`scripts/hermes`** the preferred `hermes` binary (it delegates when the last token is **`droplet`**).
...
- **Automated / non-interactive remote steps** (git pull, scripts, smoke tests): **`./scripts/droplet_run.sh '…'`** (runs as **`SSH_USER`**, no sudo to **`hermesuser`** for that invocation only).
- **Interactive Hermes on the VPS** uses **`scripts/agent-droplet`**, which runs **`./venv/bin/python -m hermes_cli.main`** on the VPS and defaults **`HERMES_DROPLET_REQUIRE_SUDO=1`**. A prior **`droplet_run`** does not disable sudo for **`hermes … droplet`**.

...
- **Short commands** (with **`PATH_add scripts`** / direnv): **`droplet`** → same as **`scripts/agent-droplet`** (remote Hermes in the VPS **`venv`**). **`droplet-run '…'`** → same as **`droplet_run.sh`** (one-off as **`SSH_USER`**, no sudo to **`hermesuser`**). Examples: **`droplet`**, **`droplet doctor`**, **`droplet-run 'cd ~/hermes-agent && git pull'`**.
- **Remote sudo password:** put **`SSH_SUDO_PASSWORD`** in the same **`~/.env/.env`** as **`SSH_*`**. Then **`hermes … droplet`** / **`droplet`** use **`sudo -S`** on the server (no interactive sudo prompt). If unset, you type sudo on the **remote** TTY after SSH.
- **Automated / non-interactive remote steps** (git pull, scripts, smoke tests): run **`./scripts/droplet_run.sh '…'`** or **`droplet-run '…'`**. That sets **`HERMES_DROPLET_REQUIRE_SUDO=0`** only for that child process (no sudo to **`hermesuser`** on the VPS). Nothing to “turn back on” afterward — the next command uses defaults again.
- **Interactive Hermes on the VPS** (`hermes tui droplet`, **`droplet`**, etc.): uses **`scripts/agent-droplet`**, which runs **`./venv/bin/python -m hermes_cli.main`** on the VPS and defaults **`HERMES_DROPLET_REQUIRE_SUDO=1`**. It does **not** inherit a stray **`HERMES_DROPLET_REQUIRE_SUDO`** from the parent shell, so a prior **`droplet_run`** does not disable sudo for **`droplet`** / **`hermes … droplet`**.

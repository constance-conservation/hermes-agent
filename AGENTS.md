...
- **Tailscale SSH timeout, LAN works:** set **`MACMINI_SSH_LAN_IP`** (mini’s LAN IPv4) for fallback. If **`MACMINI_SSH_HOST`** is a stale **100.x** Tailscale address, set **`MACMINI_SSH_TRY_LAN_FIRST=1`** in the same **`~/.env/.env`** so **`ssh_operator.sh`** / **`ssh-operator-breakglass.sh`** try LAN before Tailscale (no 8s wait when you are on the same LAN). To refresh **`MACMINI_SSH_HOST`**, on the mini run **`tailscale ip -4`** (or from the repo: **`./scripts/core/operator_print_mini_tailscale_ip.sh`**) and update the env file.

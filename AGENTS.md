...
**Shell outside the repo (Mac/Linux):** To make **`hermes`** always use this checkout’s venv and shim (including **`hermes … droplet`**), source **`scripts/shell/hermes-env.sh`** from **`~/.zshrc`** / **`~/.bashrc`** after setting **`HERMES_AGENT_REPO`** if your clone is not **`$HOME/hermes-agent`**. That defines **`hermes()`** (delegates to **`scripts/hermes`**), **`droplet()`** (plain **`ssh hermesuser@…`** via **`scripts/ssh_droplet_hermesuser_direct.sh`** — pubkey must be in **`hermesuser`**’s **`authorized_keys`**), and **`droplet_sudo()`** (admin **`SSH_USER`** then **`sudo`** to **`hermesuser`**, **`scripts/ssh_droplet_user.sh`**). All need **`~/.env/.env`** (`SSH_PORT`, host, and for sudo path **`SSH_USER`**) + **`~/.env/.ssh_key`**. On the VPS, **`agent-droplet`** runs **`./venv/bin/python -m hermes_cli.main`** and exports **`VIRTUAL_ENV`** + **`PATH`** so child processes keep using that venv.

...
- **Sudo on the mini (not the droplet):** **`ssh_operator.sh`** does **not** use **`sudo -S`** or a password from **`~/.env/.env`**. It uses **`ssh -tt`** and **`RequestTTY=force`**, and runs **`sudo -k`** at the start of each remote command to clear cached sudo credentials (opt out: **`HERMES_OPERATOR_SKIP_SUDO_K=1`**). We do **not** run **`sudo -v`** by default — a failing **`sudo -v`** would close the SSH session immediately for users without **`sudo`** or a usable TTY. Optional best-effort: **`HERMES_OPERATOR_SUDO_VERIFY_SOFT=1`** runs **`sudo -v || true`**. If **`sudo`** never prompts when you run it interactively, check **`/etc/sudoers`** (**`NOPASSWD`**) and **`/etc/pam.d/sudo`** (**`pam_tid`** / Touch ID). Unattended: **`HERMES_OPERATOR_SSH_NO_TTY=1`**.

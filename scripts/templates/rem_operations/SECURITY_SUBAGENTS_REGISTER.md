...
|-------|----------------|------|---------------|-------------------------|
| AG-004 | `ag-sec-preflight` | Startup Preflight Security Agent | Phase 4 #4 | `canonical-ai-agent-security-policy.md` + `role-prompts/security-foundation-agents-role-prompts.md` § AG-004 |
| AG-006 | `ag-sec-drift` | Continuous Drift and Monitoring Agent | Phase 4 #5 | same file § AG-006 |
| AG-007 | `ag-sec-filesystem` | Filesystem and Execution Security Agent | Phase 4 #6 | same file § AG-007 |
| AG-008 | `ag-sec-browser` | Browser and Web Security Agent | Phase 4 #7 | same file § AG-008 |
| AG-009 | `ag-sec-integration` | Integration and Identity Security Agent | Phase 4 #8 | same file § AG-009 + allowlists / `gateway/run.py` |
| AG-010 | `ag-sec-prompt-memory` | Prompt Injection and Memory Defense Agent | Phase 4 #9 | same file § AG-010 |
| AG-011 | `ag-sec-exfiltration` | Outbound Exfiltration Guard Agent | Phase 4 #10 | same file § AG-011 |
| AG-012 | `ag-sec-supply-chain` | Patch, Dependency, and Supply-Chain Security Agent | Phase 4 #11 | same file § AG-012 + `SKILL_INVENTORY_REGISTER.md` |
| AG-013 | `ag-sec-incident` | Incident Response Agent | Phase 4 #12 | same file § AG-013 + `INCIDENT_REGISTER.md`, `SECURITY_ALERT_REGISTER.md` |

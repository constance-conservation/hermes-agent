...
_operator_wrap_cmd_with_venv() {
  ...
  pre+='if [[ "${HERMES_REMOTE_VENV_QUIET:-0}" != "1" ]]; then command -v python >/dev/null 2>&1 && printf "\033[38;5;141m(venv)\033[0m \033[38;5;141m(venv)\033[0m  ssh %s\n    python=%s\n" "${HERMES_OPERATOR_SSH_DST:-}" "$(command -v python)" >&2; fi; '

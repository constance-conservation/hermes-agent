...
class AIAgent:
    ...
    def __init__(
        ...
        # CLI /models manual route: skip dynamic tier pick for this user turn (cleared in run_conversation).
        self._skip_per_turn_tier_routing = bool(skip_per_turn_tier_routing)
        # When True, do not rewrite model/runtime for OPM (enforce + per-iteration reconcile).
        # Set from the same turn_route flag as _skip_per_turn_tier_routing; not cleared in run_conversation.
        self._defer_opm_primary_coercion = bool(skip_per_turn_tier_routing)
        ...
        try:
            from agent.openai_primary_mode import coerce_opm_disallowed_routing_slugs, opm_coercion_effective

            if self.model and opm_coercion_effective(self):
        ...
        try:
            ...
            if (
                not opm_manual_override_active(self)
                and opm_enabled(self)
        ...
        # Manual /models pipeline: do not attach free_model_routing fallback chains (often Gemini
        # then native GPT) — the user-chosen stack should not auto-hop providers on errors.
        if skip_per_turn_tier_routing:
            self._fallback_chain = []
    ...
    def _tier_targets_openai_native_consultant(self, model_id: str) -> bool:
        ...
        if getattr(self, "_defer_opm_primary_coercion", False):
            return False
    ...
    def _reconcile_runtime_after_tier_model_change(self) -> None:
        ...
        Manual ``/models`` routes (OpenRouter shortcuts, etc.) set ``_defer_opm_primary_coercion``;
        do not rewrite the client to native OpenAI just because the slug looks like ``openai/gpt-5.4``.
        """
        if getattr(self, "_defer_opm_primary_coercion", False):
            return
    ...
    def rebind_from_cli_turn_route(self, turn_route: Dict[str, Any]) -> None:
        """Align model + HTTP client with CLI ``turn_route`` for sticky ``/models`` picks.

        Reused ``AIAgent`` instances can drift (wrong ``api_mode`` from YAML on OpenRouter,
        fallback swaps, etc.). Call each user turn when ``skip_per_turn_tier_routing`` is set.
        """
        if not isinstance(turn_route, dict) or not turn_route.get("skip_per_turn_tier_routing"):
            return
        if getattr(self, "api_mode", None) == "anthropic_messages":
            return

        m = (turn_route.get("model") or "").strip()
        rt = turn_route.get("runtime") or {}
        want_provider = (rt.get("provider") or "").strip()
        want_base = (rt.get("base_url") or "").strip().rstrip("/")
        want_key = str(rt.get("api_key") or "")
        want_mode = rt.get("api_mode") or "chat_completions"
        if not want_mode:
            want_mode = "chat_completions"
        if "openrouter.ai" in want_base.lower() and want_mode == "codex_responses":
            want_mode = "chat_completions"

        if not m or not want_base:
            logger.warning("rebind_from_cli_turn_route: missing model or base_url, skipping")
            return
        ...
        self.base_url = want_base
        self.api_key = want_key
        self.acp_command = rt.get("command")
        self.acp_args = list(rt.get("args") or [])

        want_mode_final = want_mode
        if want_mode_final == "chat_completions" and self._is_direct_openai_url(want_base):
            want_mode_final = "codex_responses"
        self.api_mode = want_mode_final
        ...
        client_kwargs: Dict[str, Any] = {"api_key": want_key, "base_url": want_base}
        if want_provider == "copilot-acp":
            client_kwargs["command"] = self.acp_command
            client_kwargs["args"] = list(self.acp_args or [])
        btl = want_base.lower()
        if "openrouter" in btl:
            client_kwargs["default_headers"] = {
                "HTTP-Referer": "https://hermes-agent.nousresearch.com",
                "X-OpenRouter-Title": "Hermes Agent",
                "X-OpenRouter-Categories": "productivity,cli-agent",
            }
        elif "api.githubcopilot.com" in btl:
            from hermes_cli.models import copilot_default_headers

            client_kwargs["default_headers"] = copilot_default_headers()
        elif "api.kimi.com" in btl:
            client_kwargs["default_headers"] = {"User-Agent": "KimiCLI/1.3"}

        if "openrouter" in btl and "claude" in m.lower():
            headers = dict(client_kwargs.get("default_headers") or {})
            _FINE_GRAINED = "fine-grained-tool-streaming-2025-05-14"
            existing_beta = headers.get("x-anthropic-beta", "")
            if _FINE_GRAINED not in existing_beta:
                headers["x-anthropic-beta"] = (
                    f"{existing_beta},{_FINE_GRAINED}" if existing_beta else _FINE_GRAINED
                )
                client_kwargs["default_headers"] = headers
        ...
        if old_client is not None:
            try:
                self._close_openai_client(old_client, reason="cli_turn_route_rebind", shared=True)
            except Exception:
                logger.debug("close client before cli rebind failed", exc_info=True)
        ...
        try:
            ...
            logger.debug("apply_client_headers after cli rebind failed", exc_info=True)
        self.client = self._create_openai_client(client_kwargs, reason="cli_turn_route_rebind", shared=True)

        is_openrouter = self._is_openrouter_url()
        is_claude = "claude" in (self.model or "").lower()
        self._use_prompt_caching = (is_openrouter and is_claude) or False
        ...
        if _cc is not None:
            ...
            _cc.model = self.model
            _cc.base_url = self.base_url
            _cc.api_key = self.api_key
            _cc.provider = self.provider
            _cc.context_length = _ctx
            try:
                _cc.threshold_tokens = int(_ctx * float(_cc.threshold_percent))
            except Exception:
                pass
        ...
        self._primary_runtime = {
            ...
            "client_kwargs": dict(self._client_kwargs),
            "use_prompt_caching": self._use_prompt_caching,
            "compressor_model": _cc.model if _cc is not None else self.model,
            "compressor_base_url": _cc.base_url if _cc is not None else self.base_url,
            "compressor_api_key": getattr(_cc, "api_key", self.api_key) if _cc is not None else self.api_key,
            "compressor_provider": _cc.provider if _cc is not None else self.provider,
            "compressor_context_length": _cc.context_length if _cc is not None else 0,
            "compressor_threshold_tokens": _cc.threshold_tokens if _cc is not None else 0,
        }
    ...
    def _opm_reconcile_primary_if_disallowed(self) -> None:
        ...
        try:
            ...
            if not opm_coercion_effective(self):
                return
    ...
    def _try_activate_fallback(self, *, triggered_by_rate_limit: bool = False) -> bool:
        ...
        try:
            from agent.openai_primary_mode import (
                manual_pipeline_no_provider_fallback_enabled,
                opm_manual_override_active,
            )

            if opm_manual_override_active(self) and manual_pipeline_no_provider_fallback_enabled():
                return False
        except Exception:
            pass
        ...
        try:
            ...
            if (
                not opm_manual_override_active(self)
                and opm_enabled(self)
                and model_id_contains_disallowed_family(fb_model)
            ):
                logging.warning(
    ...
    def _return_with_turn_done_notify(self, result: Dict[str, Any]) -> Dict[str, Any]:
        """Fire optional turn-done HTTP notify for any ``run_conversation`` dict return (incl. early exits)."""
        try:
    ...
    def run_conversation(
        ...
        try:
            ...
            try:
                ...
                _trace_gate = getattr(self, "verbose_logging", False) or (
                    _os_pre.environ.get("HERMES_PRE_LLM_ROUTING_TRACE", "").strip().lower()
                    in ("1", "true", "yes", "on")
                )
                if _trace_gate:
                ...
                logger.debug("pre_llm_api_stack trace failed", exc_info=True)
                    ...
                    try:
                        ...
                        if response_invalid:
                            ...
                            if retry_count >= max_retries:
                                ...
                                return self._return_with_turn_done_notify({
                                    "messages": messages,
                                    "completed": False,
            ...
            return self._return_with_turn_done_notify(result)
        finally:
            try:

...
| Slack stops working while WhatsApp/Telegram still fine | If you use a **shell watchdog** that reads `gateway_state.json`, it must **not** require every platform to be `connected`. When WhatsApp is `fatal`/`reconnecting`, Slack can still be healthy. Use `hermes gateway watchdog-check` (exit 0 = OK) from the repo venv instead of “all platforms connected”. |

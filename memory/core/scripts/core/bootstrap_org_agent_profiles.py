...
def _find_repo_root() -> Path:
    # scripts/core/ is typically a symlink into memory/core/scripts/core, so .resolve()
    # on __file__ lands under memory/core — wrong ROOT. Walk up from the unresolved path
    # and use pyproject.toml as the repo-root sentinel.
    start = Path(__file__).absolute()
    for cand in [start.parent.parent.parent, *start.parents]:
